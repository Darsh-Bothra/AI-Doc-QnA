...
class RetrievalService:
    ...
    def retrieve(self, question: str, *, user_id: int, document_id: int | None = None, limit: int = 5) -> list[dict]:
        vector = self.embeddings.get_embeddings([question])[0]

        return self.qdrant.search(vector, user_id=user_id, document_id=document_id, limit=limit)
